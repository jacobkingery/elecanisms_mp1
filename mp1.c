...
void mtr_setVelocity(WORD velocity) {
    // LSB is sacrificed from speed precision to encode for direction
    uint16_t speed = velocity.w & 0xFFFE;
    uint8_t direction = velocity.b[1] & 1;

    md_velocity(&mdp, speed, direction);
}
...
void VendorRequests(void) {
    ...
    switch (USB_setup.bRequest) {
        case ENC_READ_REG:
            ...
            break;
        case MTR_SET_VEL:
            mtr_setVelocity(USB_setup.wValue);
